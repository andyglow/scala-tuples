...
private[macros] trait Extractors { this: HasLog with HasContext =>
  ...
  protected def isTuple(tpe: Type): Boolean = tpe.typeSymbol.fullName.startsWith("scala.Tuple")
  ...
  protected final object CaseClass {

    // TODO: add support for case classes defined in method body

    def lookupCompanionOf(clazz: Symbol): Symbol = clazz.companion
